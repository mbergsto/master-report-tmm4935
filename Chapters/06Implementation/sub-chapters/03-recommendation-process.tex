...
filter, since all articles in the knowledge graph are tagged with 
...
A filter is also applied to remove overly general methods. 
This means that if a specific neural 
network variant is already present in the candidate set from the 
same article, the generic \texttt{neural\_network} method is 
removed not counted to reduce noise. In the end, candidates are sorted by how well they 
