...
ML Ontology was selected as the basis for MLguide's knowledge graph because it 
provides the broadest coverage of ML concepts among the ontologies 
reviewed in Section~\ref{sec:ml-ontology-structure-and-content}. 
Reusing and extending an existing ontology rather than building one from scratch 
is a recommended approach in ontology development
\parencite{noy2001ontology_development_101_guide_creating_first_ontology}.
It is open source and published under an MIT 
...
instances under a common family label, and helps selecting notebook 
...
tasks a method can be used for. Adding article relations 
directly to these instances would mix 
...
identified by its Digital Object Identifier (DOI) and title. 
The article instance is linked to 
...
\texttt{skos:exactMatch}.
The \texttt{used\_for} relation links the approach instance to the \texttt{ML\_task} 
instance for \texttt{regression}, which belongs to \texttt{supervised\_learning} 
via \texttt{belongs\_to}. The \texttt{has\_family} relation links the task instance
to the \\ 
...
to new instances via \texttt{rdfs:seeAlso}, as in the original ontology 
...
by similarity between labels and whether the Wikidata 
...
The knowledge used to update the knowledge graph
...
\begin{xltabular}{\textwidth}{>{\raggedright\arraybackslash}p{4cm}Xr}
...
The PostgreSQL database stores article data as its primary function 
but also contains tables that support other parts of MLguide, 
shown in Figure~\ref{fig:psql-er-diagram}. The database is also referred to as 
the article database when only the article tables are relevant.
...
Article data is stored across several tables in PostgreSQL. 
The \texttt{article\_abstracts} 
table stores the raw abstract text for each article, used during 
reranking of methods. The \\ \texttt{abstract\_embeddings} and 
...
similarity search via pgvector. The 
...
table is used by the article 
retrieval pipeline to record articles that were skipped in 
earlier runs, so that they can be excluded without reprocessing 
in subsequent runs. 