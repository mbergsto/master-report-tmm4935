...
\textit{Explicit} means that the concepts used and the constraints on their
use are defined explicitly.
...
In practical terms, an ontology defines a common vocabulary for a domain,
including machine-interpretable definitions of its basic concepts and the
...
commitment needed for its intended purpose, meaning it should make as
few claims about the world as necessary. This connects to a
...
is needed for the application at hand. Expanding scope beyond what the
application requires adds complexity without benefit, attaching
properties to concepts that are irrelevant for the intended use
\parencite{noy2001ontology_development_101_guide_creating_first_ontology}.
...
\parencite{noy2001ontology_development_101_guide_creating_first_ontology}, 
but is difficult to achieve in non-semantic representations that lack 
a formally shared, machine-interpretable vocabulary 
...
\parencite{humm2026industry_ready_machine_learning_ontology}.
Making these associations explicit allows methods to be compared and 
chosen in a structured way. 
\textcite{keet2015data_mining_optimization_ontology} motivate their 
ontology for data mining on these grounds. Traditional meta-learning 
treated learning algorithms as black boxes, correlating observed 
performance with data characteristics alone, while an ontology allows 
the internal characteristics of algorithms to be represented 
explicitly and reasoned over.
