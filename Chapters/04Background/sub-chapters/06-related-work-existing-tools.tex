...
(AutoML), which automatically selects, composes, and
...
knowledge graph of AI tasks, benchmark results, and 
...
\textcite{park2025chain_of_retrieval} address the task of 
...
\parencite{troumpoukis2025deployai_ai_on_demand_platform}. 
The platform targets SMEs, researchers, and public sector 
...
flowcharts and AutoML systems support method selection but 
rely mainly on performance criteria, treat the process as a 
black box, or assume that the task and a labelled dataset are 
already defined, and so give little help with the earlier step 
of understanding the problem and choosing a general approach. 
The structured frameworks and knowledge representations 
address this step more directly, by characterising methods and 
matching them to a described problem, but they depend on 
method knowledge that is populated manually, are demonstrated 
on a small number of methods or a single use case, and require 
the user to formalise the problem in a fixed structure. 
Semantic retrieval from scientific literature offers a way to 
match a free-text problem description against a large body of 
methods without manual formalisation, but existing systems 
apply it to exploratory search or to retrieving tables or 
related papers, rather than to method selection. This thesis 
addresses these gaps together, and the approach is described 
