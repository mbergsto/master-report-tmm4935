...
the thesis builds on. It first describes the main machine learning paradigms and 
tasks associated with them, then presents a selection of common 
machine learning methods with emphasis on attributes that distinguish them. 
...
Classification assigns an input to one of a fixed set of discrete 
classes, whereas regression predicts a continuous numerical value
\parencites[3]{bishop2006pattern_recognition_machine_learning}[Chapter~5]{Goodfellow-et-al-2016}.
...
Clustering groups data points so that the members 
of one group resemble each other more closely than they resemble the members of other groups
\parencite{sarker2021machine_learning_algorithms_real_world_applications_research_directions}. 
Dimensionality reduction lowers the number of features, 
so the data becomes easier for people to interpret and cheaper to process
\parencite{sarker2021machine_learning_algorithms_real_world_applications_research_directions}. 
Anomaly detection identifies data points that depart from the 
normal or expected pattern, and is used in applications such as 
fault detection 
\parencite{chandola2009_anomaly_detection_survey}.
It is also worth noting that the boundary between supervised and unsupervised learning 
...
they learn. Value-based methods learn a value function first and obtain the policy 
from it, either directly or as an implicit consequence. Policy-based methods skip 
the value function and optimise the policy on its own
...
literature, but not every method is captured by it. As mentioned above, 
the boundary 
...
population-based metaheuristics used for optimisation
\parencite{corne2025_ea}. They are not defined by the type of
supervision received during training, so they do not fit naturally
into this division. Instead, they are 
...
A Gaussian mixture model (GMM) is a probabilistic clustering model. 
It treats the data as coming from a combination of Gaussian 
distributions whose number and parameters are estimated during 
training. Each distribution corresponds to a cluster, and a point is 
assigned based on the probability that it belongs to each one. Since 
the model estimates a density over the data, points that fall in 
low-density regions can be treated as anomalies, which makes GMMs 
applicable to anomaly detection as well as clustering 
\parencite[Chapter~9]{geron2019hands_on_machine_learning}.
...
For anomaly detection, isolation forest builds 
on the idea that abnormal data points are rare 
and easy to separate from the rest, so it splits the data at random 
until each point is isolated and treats those separated in few steps 
as anomalies 
\parencite{liu2008isolation_forest}. 
One-class SVM, a variant of the support vector machine, fits a 
boundary around the normal points, and anything that falls outside 
...
This approach is geared towards deterministic policies, and small 
...
in the machine learning literature. They are not a distinct learning paradigm, 
as they can be applied to supervised, unsupervised, and reinforcement learning tasks. 
Rather, they are a family of methods that differ from the more traditional 
methods described above in their architecture and learning approach. 
...
Neural networks are machine learning models whose design takes after the way the 
human brain is built, with processing units arranged across an input 
layer, one or more hidden layers, and an output layer
\parencite{shrestha2019review_deep_learning_algorithms_architectures}. 
They are used across many problem types, from classification and 
regression to clustering, dimensionality reduction, computer vision, 
...
Rather than connecting every neuron to all neurons in the previous 
layer, CNNs rely on local connections and weight sharing, which keeps 
the parameter count low and shortens training time 
\parencite{shrestha2019review_deep_learning_algorithms_architectures}. 
...
Long short-term memory networks (LSTMs) address this through memory 
cells together with gates that decide which information is stored, 
read, and updated 
\parencite{hochreiter1997lstm, 
shrestha2019review_deep_learning_algorithms_architectures}. 
This lets LSTMs capture long-term dependencies in sequences, which 
is why they are widely applied to time series analysis, speech 
...
described above, with a short description of their structure, their most common training type, 
